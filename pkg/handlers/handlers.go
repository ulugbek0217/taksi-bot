...
func (h Handlers) Start(ctx context.Context, b *bot.Bot, update *models.Update) {
	...
	if err != nil {
		fmt.Printf("error on func start on send message: %v", err)
		return
	}
...
// Direction handles the direction
func (h *Handlers) Direction(ctx context.Context, b *bot.Bot, update *models.Update) {
	_, err := b.SendMessage(ctx, &bot.SendMessageParams{
		ChatID: update.Message.Chat.ID,
		Text:   "Manzil tanlandi. Endi yo'lovchilar/pochta sonini kiriting.",
	})
	if err != nil {
		fmt.Printf("error on func Direction on send message: %v", err)
		return
	}
...
func (h *Handlers) CustomersQuantity(ctx context.Context, b *bot.Bot, update *models.Update) {
	_, err := b.SendMessage(ctx, &bot.SendMessageParams{
		ChatID: update.Message.Chat.ID,
		Text:   "Yo'lovchilar soni qabul qilindi. Endi telefon raqamingizni yuboring.\n\nNamuna: +998901234567",
	})
	if err != nil {
		fmt.Printf("error on func CustomersQuantity on send message: %v", err)
		return
	}
...
func (h *Handlers) Order(ctx context.Context, b *bot.Bot, update *models.Update) {
	if h.cache.status == "" {
		...
		if err != nil {
			fmt.Printf("error on func Order on use keyboard message: %v", err)
			return
		}
		return
	...
	if err != nil {
		fmt.Printf("error on func Order on sending message to admin: %v", err)
	...
	if err != nil {
		fmt.Printf("error on func Order on order received message: %v", err)
	}
}
...
func (h Handlers) BotUsersQuantity(ctx context.Context, b *bot.Bot, update *models.Update) {
	...
	if err != nil {
		fmt.Printf("error on func BotUsersQuantity on counting users: %v", err)
		return
	}

	_, err = b.SendMessage(ctx, &bot.SendMessageParams{
		ChatID: update.Message.Chat.ID,
		Text:   fmt.Sprintf("Ma'lumotlar bazasidagi foydalanuvchilar soni: %d", count),
	})
